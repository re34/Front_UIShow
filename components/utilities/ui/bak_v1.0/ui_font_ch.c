...
static lv_fs_file_t font_fd;
static uint8_t bIsFileOpen;

static uint8_t *__user_font_getdata(int offset, int size)
{
	uint32_t font_readCnt;
	
	if(bIsFileOpen == 0){
		lv_fs_open(&font_fd, "L:/myFont.bin", LV_FS_MODE_RD);
		bIsFileOpen = 1;
	}
	lv_fs_seek(&font_fd, offset, LV_FS_SEEK_SET);
	lv_fs_read(&font_fd, __g_font_buf, size, &font_readCnt);
  return __g_font_buf;
}
