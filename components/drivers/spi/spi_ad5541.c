...
static M_GpioxInfo g_tDacCsPinPool[] =
{
	{GPIOD, GPIO_PIN_2},		//HV_amp   cs1---DAC_SCAN
	{GPIOC, GPIO_PIN_11}, 		//HV_bias  cs2---DAC_BIAS
	{GPIOA, GPIO_PIN_15},		//HV_scan_bias  (拉回偏置) cs3---DAC_SCAN_BIAS
};
...
static void test_uartCmd(int argc, char **argv)
{
	if (argc != 2)
	{
		...
		if(s_Para->chipVal.scanState != tmp &&  tmp<= State_In_ScanNread_Full)
		{
			...
			rt_kprintf("receive stat = %d\n", s_Para->chipVal.scanState);
